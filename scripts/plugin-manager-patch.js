...
function packageNameOf(reference) {
 ...
 *
 * @param {string} text patch 文件全文
 * @param {Iterable<string>} available 可用的包名集合
 * @returns {{ patch: string, pruned: { id: string, name: string }[] }}
 */
function pruneUnavailableRows(text, available) {
 ...
 * @returns {{ kept: unknown[], removed: string[] }}
 */
function pruneUnavailableBundles(bundles, available) {
...
module.exports = {
  togglePluginInPatch, ensurePluginDisabledInPatch, removePluginFromPatch, hasEntryId,
  packageNameOf, pruneUnavailableRows, pruneUnavailableBundles,
};
