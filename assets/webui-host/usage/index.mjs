import { apply as applySkills } from './skills-host.mjs';
/**
 * WebUI-owned usage/account host, derived from the reviewed MIT server source.
 * See PROVENANCE.json and LICENSE. This is not a standalone Cordis plugin.
 *
 ...
 *
 * @module webui/usage-host
 */

import { homedir } from "node:os";
import { join } from "node:path";
import { readFile } from "node:fs/promises";
import { applyUsageDelta, createUsageState, mergeHoursInto, mergeInto, renderSignal, renderUsage, totalTokens, zeroBuckets } from "./usage.mjs";
import { ACCOUNT_REFRESH_MS, createAccountService, validateAccountConfig } from "./accounts.mjs";
import { queryDeepseekBilling } from "./deepseek-billing.mjs";
import { atomicJson } from "./filesystem.mjs";
...
// v7: v2 embedded streams, retry accounting, zero-based storage and fork cuts.
...
function rejectForeignWrite(req, res) {
	...
	if (req.headers.origin !== undefined) {
		try { sameOrigin = new URL(req.headers.origin).host === req.headers.host; }
		catch { sameOrigin = false; }
	}
	if (sameOrigin && isLoopbackAddress(peer) && isLoopbackHostHeader(req)) return false;
	json(res, 403, { ok: false, error: "forbidden" });
	return true;
}
...
function parseSession(raw) {
	...
	// Revisions are comparable only within one persistence service instance.
	state.kind = "restored-cache";
...
async function saveCache(ctx, cache) {
	try {
		...
		await atomicJson(path, serialized);
	} catch (error) {
 ...
 * Persisted sessions use current list() snapshots and read-only handles.
 * Changed cold logs are refolded; no legacy sequence-number translation or
 * write ownership is needed. Fork-inherited prefixes are never billed twice.
	...
	return withLock(async () => {
		...
		if (live !== void 0) {
			for (const session of live.list()) {
				...
				const previous = cache.sessions[session.id];
				const state = previous?.kind === "live"
					&& liveIdentities.get(session.id) === session
					&& previous.consumed <= events.length
					? structuredClone(previous) : createUsageState();
				applyUsageDelta(state, events.slice(state.consumed));
		...
		if (persistence !== void 0) {
			for (const snapshot of await persistence.list()) {
				...
				if (state?.kind !== "persisted" || state.revision !== snapshot.revision) {
		...
		for (const session of live?.list() ?? []) liveIdentities.set(session.id, session);
		const byDay = new Map();
...
async function writeBudget(ctx, budget) {
	try {
		...
		await atomicJson(path, { version: 1, budget, updatedAt: Date.now() });
		return true;
	} catch (error) {
 ...
 */
async function applyUsageHost(ctx, rawConfig = {}, deps = {}) {
	...
	const accounts = deps.accounts ?? createAccountService({
		...
		config,
		deps: { timeoutMs: UPSTREAM_TIMEOUT_MS, ...deps.accountDeps }
	});
...
export { applyUsageHost, USAGE_PATH, SIGNAL_PATH, DAY_SESSIONS_PATH, BUDGET_PATH, PROVIDERS_PATH, BALANCE_PATH, SUBSCRIPTIONS_PATH, ACCOUNT_PATH, CREDENTIALS_PATH, BILLING_PATH, configuredProviders, totalTokens, zeroBuckets };
