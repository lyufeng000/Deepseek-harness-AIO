...
import { unzipArchive } from "./archive.mjs";
import { assertNoLinks, atomicJson, boundedPath, removeBounded } from "./filesystem.mjs";
...
async function walkSkillDir(dir, prefix, out) {
  ...
  for (const entry of entries) {
    if (entry.isSymbolicLink()) throw new Error("linked skill paths are not supported");
...
async function readSkillMeta(root, dir) {
  const skillDir = boundedPath(root, dir);
  await assertNoLinks(join(skillDir, SKILL_FILE));
  let raw;
  try {
...
async function readBundles(root) {
  await assertNoLinks(join(root, BUNDLES_FILE));
  try {
    const parsed = JSON.parse(await readFile(join(root, BUNDLES_FILE), "utf8"));
    if (typeof parsed === "object" && parsed !== null && parsed.version === 1
        && Array.isArray(parsed.bundles) && parsed.bundles.every(bundle =>
          typeof bundle.id === "string" && typeof bundle.name === "string"
          && Array.isArray(bundle.skills) && bundle.skills.every(name => typeof name === "string"))) {
      return parsed;
    }
    throw new Error("invalid skill bundle ledger");
  } catch (error) {
...
async function writeBundles(root, file) {
  await atomicJson(join(root, BUNDLES_FILE), file);
}
...
async function assignedLedger(root, skillName, bundleId) {
  const ledger = await readBundles(root);
  if (typeof bundleId !== "string" || bundleId === "") return ledger;
...
// Stage a complete replacement and preserve the original until the ledger
// commit succeeds. Existing unmentioned files retain the old overlay semantics.
export async function commitSkill(root, skillName, files, bundleId, writeLedger = writeBundles) {
  const target = boundedPath(root, skillName);
  await assertNoLinks(target);
  const ledger = await assignedLedger(root, skillName, bundleId);
  const seen = new Set();
  for (const file of files) {
    const destination = boundedPath(target, file.name).toLowerCase();
    if (seen.has(destination)) throw new Error("duplicate skill file path");
    seen.add(destination);
  }
  const stage = boundedPath(root, `.webui-install-${randomUUID()}`);
  const backup = boundedPath(root, `.webui-backup-${randomUUID()}`);
  let hadOriginal = false;
  let installed = false;
  try {
    await mkdir(root, { recursive: true });
    try {
      if (!(await stat(target)).isDirectory()) throw new Error("skill target is not a directory");
      await walkSkillDir(target, "", []);
      hadOriginal = true;
    } catch (error) {
      if (error.code !== "ENOENT") throw error;
    }
    if (hadOriginal) await cp(target, stage, { recursive: true, errorOnExist: true, force: false });
    else await mkdir(stage);
    for (const file of files) {
      const destination = boundedPath(stage, file.name);
    ...
    try {
      ...
      if (installed) await removeBounded(root, target);
      if (hadOriginal) await rename(backup, target);
      throw error;
    }
    if (hadOriginal) await removeBounded(root, backup);
  } finally {
    await removeBounded(root, stage);
  }
}
async function installArchive(body) {
  ...
  const normalized = files.map(file => {
    // Validate BEFORE stripping the common archive prefix.
    boundedPath(root, file.name);
    if (base && !file.name.startsWith(base + "/")) throw new Error("archive has files outside its skill directory");
    return { ...file, name: base ? file.name.slice(base.length + 1) : file.name };
  });
  await commitSkill(root, skillName, normalized, body.bundleId);
...
async function installSkill(body) {
  ...
  const normalized = [];
  for (const file of files) {
    if (typeof file !== "object" || file === null) throw new Error("invalid skill file");
    const entry = file;
    const path = typeof entry.path === "string" ? entry.path : "";
    const data = typeof entry.data === "string" ? entry.data : "";
    if (path === SKILL_FILE) hasSkillFile = true;
    normalized.push({ name: path, data: Buffer.from(data, "base64") });
  }
  if (!hasSkillFile) {
...
${description}`;
    normalized.push({ name: SKILL_FILE, data: Buffer.from(text, "utf8") });
  }
  await commitSkill(root, skillName, normalized, body.bundleId);
...
async function readSkillFile(skillName, relPath) {
  ...
  if (!NAME_PATTERN.test(name2)) throw new Error("invalid skill name");
  const roots = [managedRoot(), dshRoot()];
  let dir = null;
  for (const root of roots) {
    const candidate = boundedPath(root, name2);
    await assertNoLinks(candidate);
    try {
...
export async function deleteSkill(skillName, writeLedger = writeBundles) {
  const name2 = checkedName(skillName);
  ...
  for (const root2 of roots) {
    const dir = boundedPath(root2, name2);
    await assertNoLinks(dir);
    try {
    ...
    const tombstone = boundedPath(root2, `.webui-delete-${randomUUID()}`);
    await rename(dir, tombstone);
    try {
      await writeLedger(root, {
        version: 1,
        bundles: ledger.bundles.map(candidate => ({
          ...candidate, skills: candidate.skills.filter(candidateName => candidateName !== name2)
        }))
      });
    } catch (error) {
    ...
    await removeBounded(root2, tombstone);
    return;
  }
  throw new Error(`skill ${JSON.stringify(name2)} not found`);
}
...
async function handle(ctx, req, res, bodyPromise) {
...
async function apply(ctx) {
  ctx.effect(() => ctx.webServer.register({
    ...
    path: ROUTE_PREFIX,
    handler: (req, res) => {
      // Attach body listeners now, not after waiting for another mutation.
      const body = req.method === "GET" || req.method === "DELETE"
        ? Promise.resolve({}) : readBody(req);
      body.catch(() => {});
      const current = pending.then(() => handle(ctx, req, res, body));
