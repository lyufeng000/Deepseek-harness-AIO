 ...
 * a usage sample rides a v2 `assistant/message` or `assistant/attempt`
 * embedded stream (or the explicit message `data.usage`); a repeated sample for the same
...
export function applyUsageDelta(state, events) {
	...
	for (const event of events) {
		if (event.type === "llm/retry-started"
			&& last?.key === `${event.data.turn}:${event.data.step}`) last = null;
