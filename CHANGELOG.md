...
### 修复：DeepSeek Flash 原生识图在真实实例上失效
- 根因：受控种子补丁只作用于打包产物的 profile seed 副本，而实例的
  `profiles/node_modules/@deepseek-ai/*` 是指向 `resources/app/node_modules`
  安装闭包的链接（侧车还会清理 profile 里遮蔽该闭包的包拷贝）。因此所有内核包
  补丁在用户机器上都不生效：上传图片被 API 层以 `MODEL_DOES_NOT_SUPPORT_IMAGES`
  拒绝，界面提示「当前模型不支持图片，请切换支持图片的模型」。
- 打包阶段在 staging 之后对安装闭包副本执行同一批受控补丁；`@deepseek-ai/*`
  目标自动镜像，新增内核包补丁无需手工登记，严格模式下一旦锚点漂移即阻断构建，
  避免再次出现「补丁已打、实例未生效」。
- DeepSeek 模型条目同时接受 `input` 与 `inputModalities`，缺少能力字段的条目
  不再让整份设置解析失败（设置页「获取可用模型」写回的就是 `inputModalities`）。

### 修复：首轮被拒后「新对话」卡在空会话
- 「新对话」不再复用已经发起过请求（含首轮被拒）的空白会话，避免点击后停在
  空会话不动；只复用真正没有发送过任何内容的空白会话。

## [1.3.2] — 2026-09-12

### 修复：会话切换与跨轮浏览抽动
- 移除 WebUI 与客制化插件在消息区域叠加的进场位移动画，统一为不改变布局坐标系的
  透明度过渡；切换期间不再给滚动定位所依赖的祖先节点留下 transform。
- 关闭已加载 Markdown 的固定高度离屏占位与滚动卸载，避免代码高亮、图片加载和
  节点重新挂载改变历史消息总高度；保留历史分页与明确跳转的滚动补偿。
- 会话切换恢复操作按当前会话失效旧回调，防止快速连续切换时迟到操作覆盖新会话位置。

### 修复：AGENTS.md 改为隐式指令上下文
- 首轮不再把 AGENTS.md 拼进用户消息或显示成额外气泡；使用内核的
  `agent-instructions` 隐藏投影，用户正文、复制内容和历史消息保持原样。
- 修正 anchored preset 对 `agent-instructions` 的首轮过滤，并让 `/init` 清理官方插件的
  指令版本缓存，下一次真实请求重新投影指令且不重复叠加。

### 新增：完成、中断、询问与错误分类音效
- 音效设置扩展为四类独立开关、选曲和试听，继续支持总开关、总音量和自定义目录。
- 只根据主任务最终状态播放；过滤子任务、自动重试、普通工具错误、历史重放和会话切换，
  并按回合与询问请求去重，避免同一次失败先播放完成音再播放错误音。
- 自动迁移旧完成音配置；新增分类默认沿用总静音状态，并预设水滴、铃声和短促双响。

### 修复：DeepSeek 配置、图标与 Flash 原生识图
- 官方配置统一写入 `llm-deepseek.providers.deepseek-official`；连接字段、模型字段和表单
  使用公共格式，官方适配层再把模型 `input` 转为内核运行时的 `inputModalities`。
- 升级时先备份再迁移旧配置，新结构的显式字段优先，并保留自定义参数与凭据引用。
- 补齐 `deepseek-official → deepseek` 本地图标映射；精确声明已知官方 Flash 型号支持
  图片输入，并兼容旧 Flash 名称，不再把第三方同名模型一概判定为多模态。

### 修复：插件市场清单加载失败
- 默认清单改为作者当前可读取的 `marketplace.json`，仅精确替换旧失效地址，保留用户
  自定义来源和 GitHub 仓库发现开关。
- 对成功清单做结构校验并缓存最后一次有效结果；单个来源失败时继续展示其他来源，
  全部失败时显示失败来源、状态与缓存时间，并提供重试入口。

### 修复：输入与流式滚动稳定性
- Composer 灵动岛默认禁用并保留手动启用；定位改为同帧更新，避免输入时工具控件闪烁。
- 滚轮、触摸惯性和滚动条手势期间暂停流式自动跟随，用户查看历史时不再被抢回底部。
...
### 验证：安装版应用内自动更新
- 与 1.2.2 同代码，仅提升版本号，用于在已修复 /D= 传参的 1.2.2 上验证
  “检查更新 → 下载 → 校验 → 助手安装 → 自动拉起新版”的完整链路。

## [1.2.2] — 2026-09-10

### 修复：安装版自动更新在含空格安装路径下失败
- 更新助手调用 NSIS 安装器时改为逐字传参（`windowsVerbatimArguments`），`/D=`
  不再被 Node 自动加引号；安装器先复制到无空格的事务路径再执行。此前 `/D=` 带
  引号会导致安装器未装到目标目录，进而“安装后版本与目标不符”回滚。
- 失败回滚后仍会自动拉起旧版本；成功安装则由助手拉起新版本并等待健康确认。
