...
- Composer 灵动岛默认改为禁用，保留插件管理中的手动启用；已有 profile
  首次升级时一次性迁移为禁用，之后用户手动启用不会再被同步覆盖。
- 灵动岛重新定位改为同帧原子更新：先应用新位置，再清理旧属性；相关
  DOM 变化在绘制前同步重扫，避免输入时工具短暂回到输入框。
- 新增迁移、幂等和 DOM 生命周期回归测试。

### 修复：对话界面上滑/下滑会被流式跟随抢回底部
- 回移 DeepSeek Harness 上游 2026-09-01 与 2026-09-07 的会话滚动采样修复：
  滚轮、触摸惯性和滚动条手势进行中，暂停 ResizeObserver 的自动跟随，待
  `scrollend` 或 500ms 采样窗口结束后再统一结算滚动归属。
- 区分真实阅读者手势与程序化 pinned scroll；正常流式输出仍即时跟随，用户
